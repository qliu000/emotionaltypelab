/* ---------------------------------------------
 * Emotion Type Lab – 4 fonts + static warp
 * -------------------------------------------*/
...
function drawBottomBarHelp() {
  ...
  textSize(12);
  textAlign(CENTER, CENTER);
  text("Slider 左端：原始字体 ｜ 往右：开始扭曲（静态形变）", width/2, height - 18);
}
...
function drawWarpedGlyph(polys, bbox, offset, vol, pit, fontSize) {
  ...
  for (const poly of polys) {
    ...
    for (let i = 0; i < poly.length; i++) {
      ...
      const edgeFactor = lerp(0.3, 1.0, constrain(r / maxR, 0, 1)); // 中间小、边缘大

      // 左右扭曲由 Volume 控制，上下扭曲由 Pitch 控制
